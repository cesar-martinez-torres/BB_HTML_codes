...
class CalendarBuilderApp(tk.Tk):
    ...
    # =========================
    # (NUEVO) PDF Export helpers
    # =========================
    def _pdf_type_color(self, event_type: str):
        # Mantener "colores" equivalentes a los del HTML
    ...
    def _pdf_priority_type_for_date(self, date_str: str) -> str | None:
        # Prioridad para pintar el día si hay varios tipos
        # Exam > Task > Class > Info
    ...
    def export_pdf(self):
        ...
        # Solo una hoja Ene–May (como pediste)
        try:
        ...
        header_title = (self.header_title_var.get() or "").strip()
        if not header_title:
            header_title = "Calendario del Curso"
    ...
    def _render_pdf_ene_may(self, out_path: str, year: int, header_title: str):
        ...
        c.drawString(margin, H - margin - 18, f"{header_title}")
        ...
        # Grid: 5 months in one page (2 rows: 3 top, 2 bottom)
        months = [1, 2, 3, 4, 5]
        cols = 3
        rows = 2

        grid_top = H - margin - header_h - legend_h - 8
        grid_bottom = margin
        ...
        # Day-of-week labels (Sun..Sat) like HTML
        dow = ["Dom", "Lun", "Mar", "Mié", "Jue", "Vie", "Sáb"]

        # Calendar config: week starts Sunday (firstweekday=6)
        cal = calendar.Calendar(firstweekday=6)

        today = date.today()
        today_str = self._date_str(today)

        def month_title_es(m: int) -> str:
            names = ["ENERO", "FEBRERO", "MARZO", "ABRIL", "MAYO", "JUNIO",
                     "JULIO", "AGOSTO", "SEPTIEMBRE", "OCTUBRE", "NOVIEMBRE", "DICIEMBRE"]
            return names[m-1]

        # Draw each month mini-calendar
        for idx, m in enumerate(months):
            ...
            c.rect(x0, y0, cell_w, cell_h, fill=1, stroke=1)

            # month header bar
            bar_h = 18
            c.setFillColor(rl_colors.HexColor("#f2f2f2"))
            c.rect(x0, y1 - bar_h, cell_w, bar_h, fill=1, stroke=1)

            c.setFillColor(rl_colors.black)
            c.setFont("Helvetica-Bold", 11)
            c.drawString(x0 + 6, y1 - 13, month_title_es(m))
            ...
            for i, name in enumerate(dow):
                dx = x0 + i * (cell_w / 7)
                c.drawCentredString(dx + (cell_w/7)/2, y1 - bar_h - 11, name)

            # Days grid area
            weeks = cal.monthdatescalendar(year, m)
            # Normalize to 6 rows like UI (some months can be 4-6; force 6 for consistent layout)
            while len(weeks) < 6:
                weeks.append([weeks[-1][-1] + timedelta(days=i+1) for i in range(7)])

            grid_y_top = y1 - bar_h - dow_h - 4
            grid_y_bottom = y0 + 6
            day_area_h = grid_y_top - grid_y_bottom
            day_h = day_area_h / 6
            day_w = cell_w / 7
            ...
            for wr in range(6):
                for wc in range(7):
                    dd = weeks[wr][wc]
                    rx = x0 + wc * day_w
                    ry = grid_y_top - (wr + 1) * day_h

                    in_month = (dd.month == m)
                    d_str = f"{dd.year:04d}-{dd.month:02d}-{dd.day:02d}"

                    # background: event color if any, else white, else light gray if not in month
                    fill = rl_colors.white
                    if in_month:
                        typ = self._pdf_priority_type_for_date(d_str)
                        if typ:
                            fill = self._pdf_type_color(typ) or rl_colors.white
                    else:
                        fill = rl_colors.HexColor("#f7f7f7")

                    c.setFillColor(fill)
                    c.rect(rx, ry, day_w, day_h, fill=1, stroke=1)

                    # day number
                    c.setFillColor(rl_colors.black if in_month else rl_colors.HexColor("#999999"))
                    c.setFont("Helvetica", 8)
                    c.drawString(rx + 2, ry + day_h - 10, str(dd.day))

                    # today outline (only if it's within this month box)
                    if d_str == today_str and in_month:
                        c.setStrokeColor(rl_colors.black)
                        c.setLineWidth(2)
                        c.rect(rx + 1, ry + 1, day_w - 2, day_h - 2, fill=0, stroke=1)
                        c.setLineWidth(1)
                        c.setStrokeColor(rl_colors.HexColor("#cccccc"))
    ...
    def _build_ui(self):
        ...
        # =========================
        # (NUEVO) Exportar PDF en menú Archivo
        # =========================
        filemenu.add_command(label="Exportar PDF…", command=self.export_pdf)
        filemenu.add_separator()
