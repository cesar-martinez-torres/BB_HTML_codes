...
# =========================
# (NUEVO) Frame con scroll
# =========================

class ScrollableFrame(ttk.Frame):
    """
    Contenedor con scrollbar vertical (ideal para el panel derecho).
    Usa .inner como el frame donde empacar widgets.
    """
...
class DatePicker(tk.Toplevel):
    ...
    """
    def __init__(self, master, title: str, current_date: date | None, on_pick,
...
class CalendarBuilderApp(tk.Tk):
    ...
    def _on_pick_range_end(self, d_end: date):
        ...
        label = simpledialog.askstring(
            "Etiqueta",
            f"Etiqueta para el rango ({self._date_str(start)} → {self._date_str(end)}):",
    ...
    def _remove_range(self, kind: str):
        ...
        def in_range(e: Event) -> bool:
            ed = self._parse_date(e.date_str)
            return (ed is not None) and (start <= ed <= end)

        self.events = [e for e in self.events if not (e.type == ev_type and e.title == title and in_range(e))]
    ...
    def _build_ui(self):
        ...
        # (CAMBIO) Panel derecho con scroll
        right = ScrollableFrame(main)
        right.pack(side="right", fill="y", padx=(10, 0))
        container = right.inner  # aquí van TODOS los widgets del panel derecho
        ...
        edit = ttk.LabelFrame(container, text="Editar evento seleccionado")
        ...
        # Nuevo evento manual (AHORA SI VISIBLE CON SCROLL)
        add = ttk.LabelFrame(container, text="Nuevo evento manual")
    ...
    def add_event(self):
        ...
        self._refresh_holidays_list()
        self._render_month()
        self._set_status(f"Evento agregado en {d}")
    ...
    # =========================
    # Export PDF (Ene–May, 1 hoja, layout columnas)
    # =========================
    ...
    # ===========
    # (CAMBIO) Limpieza SOLO para PDF: quitar prefijos y evitar cortes
    # ===========
    def _pdf_titles_for_date(self, date_str: str) -> list[str]:
        ...
        for e in evs_sorted:
            ...
            # Quitar prefijos técnicos SOLO para PDF
            if title.startswith("[CIERRE]"):
                title = title.replace("[CIERRE]", "", 1).strip()
            if title.startswith("[VACACIONES]"):
                title = title.replace("[VACACIONES]", "", 1).strip()
            if title.startswith("[FESTIVO]"):
                title = title.replace("[FESTIVO]", "", 1).strip()

            # Evitar salto feo en "y Tareas" / "y Tarea" usando NBSP
            title = title.replace(" y Tareas", " y\u202FTareas")
            title = title.replace(" y Tarea", " y\u202FTarea")
        ...
        for i, m in enumerate(months):
            ...
            for day in range(1, rows + 1):
                ...
                if day <= last_day:
                    ...
                    text_right_x = col_x + col_w - pad  # alineación derecha
